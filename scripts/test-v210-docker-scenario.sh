...
create_route_state() {
  ...
  http_path_response="$(api_request POST /proxy/paths "$access_token" "$tenant_id" "{\"chainId\":\"${chain_id}\",\"name\":\"HTTP access\",\"mode\":\"forward\",\"protocol\":\"http\",\"serviceType\":\"http_forward_proxy\",\"remoteProtocol\":\"\",\"listenHost\":\"127.0.0.1\",\"listenPort\":${node_http_host_port},\"targetProtocol\":\"http\",\"targetHost\":\"example.test\",\"targetPort\":80,\"targetSni\":\"\",\"tlsMode\":\"\",\"authMode\":\"proxy_token\",\"options\":{}}")"
  http_path_id="$(printf '%s' "$http_path_response" | json_get data.id)"
  ssh_path_response="$(api_request POST /proxy/paths "$access_token" "$tenant_id" "{\"chainId\":\"${chain_id}\",\"name\":\"SSH access\",\"mode\":\"tcp\",\"protocol\":\"tcp\",\"serviceType\":\"tcp_access\",\"remoteProtocol\":\"ssh\",\"listenHost\":\"127.0.0.1\",\"listenPort\":${node_tcp_host_port},\"targetProtocol\":\"ssh\",\"targetHost\":\"ssh.example.test\",\"targetPort\":22,\"targetSni\":\"\",\"tlsMode\":\"\",\"authMode\":\"proxy_token\",\"options\":{}}")"
  ssh_path_id="$(printf '%s' "$ssh_path_response" | json_get data.id)"
  rdp_path_response="$(api_request POST /proxy/paths "$access_token" "$tenant_id" "{\"chainId\":\"${chain_id}\",\"name\":\"RDP access\",\"mode\":\"tcp\",\"protocol\":\"tcp\",\"serviceType\":\"tcp_access\",\"remoteProtocol\":\"rdp\",\"listenHost\":\"127.0.0.1\",\"listenPort\":${node_tcp_host_port},\"targetProtocol\":\"rdp\",\"targetHost\":\"rdp.example.test\",\"targetPort\":3389,\"targetSni\":\"\",\"tlsMode\":\"\",\"authMode\":\"proxy_token\",\"options\":{}}")"
